...
let previousAdminPipe: string | undefined;
...
async function setupGatewayTestHome() {
  ...
  previousAdminPipe = process.env.OPENCLAW_ADMIN_PIPE;
  ...
  // Avoid collisions with a locally-running gateway that binds the default admin pipe name.
  // Also prevents worker-to-worker collisions when vitest runs suites in parallel.
  const worker =
    String(process.env.VITEST_POOL_ID || process.env.VITEST_WORKER_ID || "0").trim() || "0";
  const suffix = `${process.pid}-${worker}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  process.env.OPENCLAW_ADMIN_PIPE =
    process.platform === "win32"
      ? `\\\\.\\pipe\\openclaw-admin-test-${suffix}`
      : path.join(os.tmpdir(), `openclaw-admin-test-${suffix}.sock`);
}
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousAdminPipe === undefined) {
      delete process.env.OPENCLAW_ADMIN_PIPE;
    } else {
      process.env.OPENCLAW_ADMIN_PIPE = previousAdminPipe;
    }
